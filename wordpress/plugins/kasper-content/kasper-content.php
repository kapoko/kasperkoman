...
final class Kasper_Koman_Content {
    ...
    public static function register_content() {
        ...
        add_image_size('release-cover', 1200, 1200, true);
    }
    ...
    private static function field($post, $key, $label, $type = 'text') {
        ...
        if ($type === 'textarea') printf('<textarea class="widefat" rows="5" id="%1$s" name="%1$s">%2$s</textarea>', esc_attr($key), esc_textarea($value));
        else printf('<input class="widefat" type="%1$s" id="%2$s" name="%2$s" value="%3$s">', esc_attr($type), esc_attr($key), esc_attr($value));
        echo '</p>';
    }
    ...
    public static function save_fields($post_id) {
        if (!isset($_POST['kk_content_nonce']) || !wp_verify_nonce($_POST['kk_content_nonce'], 'kk_content') || wp_is_post_autosave($post_id) || wp_is_post_revision($post_id)) return;
    ...
    public static function request_build() { self::$build_needed = true; }
    public static function request_build_for_meta($meta_id, $post_id) {
        if (in_array(get_post_type($post_id), ['release', 'gig'], true)) self::$build_needed = true;
    }
    public static function trigger_build() {
        if (!self::$build_needed || defined('KK_IMPORTING')) return;
    ...
    public static function import($args, $assoc_args) {
        ...
        foreach ($releases as $release) {
            ...
            if (isset($media_by_release[$release_id])) $release_media_ids[self::oid($media_by_release[$release_id]['_id'] ?? '')] = true;
        }
    ...
    private static function read_ndjson($file) {
        if (!is_readable($file)) WP_CLI::error("Missing import file: $file");
    ...
    private static function oid($value) { return is_array($value) ? ($value['$oid'] ?? '') : $value; }
    private static function date($value) { return is_array($value) ? substr($value['$date'] ?? '', 0, 10) : substr((string) $value, 0, 10); }
    ...
    private static function import_releases($releases, $media_by_release, $source) {
        ...
        foreach ($releases as $release) {
            ...
            if (!$post_id) $post_id = wp_insert_post(['post_type' => 'release', 'post_status' => 'publish', 'post_title' => $release['title'] ?? 'Untitled release']);
            update_post_meta($post_id, 'strapi_id', $id);
            foreach (['subtitle' => 'release_subtitle', 'label' => 'release_label', 'links' => 'release_links', 'originalArtist' => 'release_original_artist'] as $old => $new) update_post_meta($post_id, $new, $release[$old] ?? '');
            update_post_meta($post_id, 'release_date', self::date($release['releaseDate'] ?? ''));
            if (isset($media_by_release[$id]) && !has_post_thumbnail($post_id)) self::import_cover($post_id, $media_by_release[$id], "$source/uploads");
            $count++;
        }
        return $count;
    }
    ...
    private static function import_gigs($gigs) {
        ...
        foreach ($gigs as $gig) {
            ...
            if (!$post_id) $post_id = wp_insert_post(['post_type' => 'gig', 'post_status' => 'publish', 'post_title' => $gig['title'] ?? 'Untitled gig']);
            update_post_meta($post_id, 'strapi_id', $id);
            foreach (['city' => 'gig_city', 'countryCode' => 'gig_country_code', 'url' => 'gig_url', 'venue' => 'gig_venue'] as $old => $new) update_post_meta($post_id, $new, $gig[$old] ?? '');
    ...
    private static function import_nonrelease_media($uploads, $release_media_ids, $source) {
        foreach ($uploads as $upload) if (empty($release_media_ids[self::oid($upload['_id'] ?? '')])) self::import_cover(0, $upload, "$source/uploads");
    }
...
if (defined('WP_CLI') && WP_CLI) WP_CLI::add_command('kasper import', [Kasper_Koman_Content::class, 'import']);
